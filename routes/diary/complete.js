...
router.put('/', upload.single('diary_img'), authUtil.isLoggedin, async(req, res) => {
    ...
    if(req.file == undefined){//이미지 없음
        const updateDiaryResult = await db.queryParam_Parse(updateDiaryQuery,[req.body.diary_content, req.body.weatherIdx, null, req.decoded.idx, req.body.date+'%']);
        ...
        const updateDiaryResult = await db.queryParam_Parse(updateDiaryQuery,[req.body.diary_content, req.body.weatherIdx, diary_img, req.decoded.idx, req.body.date+'%']);
        if (updateDiaryResult.length == 0) { 
            console.log(3);
            res.status(200).send(util.successFalse(statusCode.DB_ERROR, resMessage.UPDATE_DIARY_FAIL));
        } else {
